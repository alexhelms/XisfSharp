...
public class XisfImageTests
{
    ...
    [TestMethod]
    public async Task CaseInsensitiveEnumParsing()
    {
        string xml =
            """
            <xisf xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.pixinsight.com/xisf http://pixinsight.com/xisf/xisf-1.0.xsd" version="1.0" xmlns="http://www.pixinsight.com/xisf">
            	<Image geometry="40:30:1" sampleFormat="UINT16" colorSpace="GRAY" location="embedded" imageType="FLAT" pixelStorage="PLANAR">
                    ...
                    <Property id="TestProp" type="FLOAT32" value="1.234"/>
            	</Image>
            </xisf>
            """;

        using var stream = TestHelpers.CreateXisfStream(xml);
        ...
        image.Channels.ShouldBe(1);
        image.SampleFormat.ShouldBe(SampleFormat.UInt16);
        image.ColorSpace.ShouldBe(ColorSpace.Gray);
        image.ImageType.ShouldBe(ImageType.Flat);
        image.Properties.Count.ShouldBe(1);
        image.Properties[0].Id.ShouldBe("TestProp");
